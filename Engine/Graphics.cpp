...
void Graphics::DrawSpriteGhost(int x, int y, RectI srcRect, const Surface & s, Color chroma)
{
...
void Graphics::DrawSpriteGhost(int x, int y, RectI srcRect, const RectI & clip, const Surface & s, Color chroma)
{
	...
	for (int sy = srcRect.top; sy < srcRect.bottom; ++sy)
	{
		for (int sx = srcRect.left; sx < srcRect.right; ++sx)
		{
			...
			if (srcPixel != chroma)
			{
				const Color backPixel = GetPixel(x + sx - srcRect.left, y + sy - srcRect.top);
				const Color blendedPixel = {
					unsigned char((s.GetPixel(sx, sy).GetR() + backPixel.GetR()) / 2),
					unsigned char((s.GetPixel(sx, sy).GetG() + backPixel.GetG()) / 2),
					unsigned char((s.GetPixel(sx, sy).GetB() + backPixel.GetB()) / 2)
				};
